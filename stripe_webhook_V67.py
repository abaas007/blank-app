"""
RentFlow V67 Stripe Webhook Service

Run separately from Streamlit:
    pip install fastapi uvicorn stripe supabase
    uvicorn stripe_webhook_V67:app --host 0.0.0.0 --port 8001

Stripe webhook destination:
    POST /stripe/webhook

IMPORTANT FOR TENANT RENT:
    Configure the Stripe webhook destination to receive events from
    connected accounts as well as the RentFlow platform account.
...
if not secrets_path.exists():
    raise RuntimeError(
        ".streamlit/secrets.toml was not found."
    )
...
PRICE_TO_PLAN = {
    secrets.get("STRIPE_PRICE_STARTER"): "Starter",
    secrets.get("STRIPE_PRICE_PROFESSIONAL"): "Professional",
    secrets.get("STRIPE_PRICE_PORTFOLIO"): "Portfolio",
}
...
def get_event_connected_account(event):
    """
    Stripe Connect events delivered for connected accounts include the
    connected account ID on the Event's `account` field.
    """
    try:
...
def is_tenant_rent_checkout(session_dict):
    """
    V67 sessions include payment_kind=tenant_rent.

    The metadata-shape fallback supports Checkout Sessions created by V65-V66D
    immediately before the V67 deployment.
    """
...
def record_tenant_rent_payment(session_obj, connected_account_id, event_created=None):
    """
    Record one successful connected-account Checkout Session into RentFlow.

    Database idempotency:
      - application lookup by stripe_session_id
      - existing T4 unique index payments_stripe_session_uidx
    """
    ...
    try:
        ...
        # The browser-return fallback and webhook can race. The existing
        # unique index on stripe_session_id is the final idempotency guard.
        existing_after_race = (
            supabase_admin
            .table("payments")
            .select("payment_id")
